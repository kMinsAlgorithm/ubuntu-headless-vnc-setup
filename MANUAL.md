...
5. 외부 접속 경로를 구성하고 실제 외부망에서 시험한다.
...
| 외부 접속 | 공인 IP에서 VNC를 사용한 기록 | 공유기·방화벽·VPN·SSH 경로 기록 |
...
| 외부 경로 | 공유기 규칙 미확인 | 공인 주소·NAT·VPN·SSH 조사 |
...
기존 접속을 유지한 채 별도 터미널에서 SSH 접속을 확인합니다. 그래픽 설정 적용에 실패했을 때 그 경로로 원복할 수 있어야 합니다. SSH가 없으면 로컬 콘솔 등 복구 경로를 준비합니다.
...
```bash
apt-cache policy x11vnc x11-xserver-utils x11-utils openssh-server mesa-utils
```
...
```bash
sudo apt update
sudo apt install x11vnc x11-xserver-utils x11-utils openssh-server mesa-utils
...
- `-rfbport` → 선택한 포트.
- `-listen 0.0.0.0` → 10장에서 선택한 연결 방식의 바인딩.
...
없는 도구나 권한 부족은 조사 결과에 기록합니다. `0.0.0.0:5900`만으로 공유기의 외부 개방까지 판정하지 않습니다. IPv6 소켓도 확인합니다.

기준 PC에는 외부 IP의 VNC 사용 기록이 있지만 공유기 규칙은 미확인입니다. Tailscale 서비스는 실행 중이어도 `NeedsLogin`이며 VPN IP가 없습니다. Tailscale 기반 외부 접속으로 기록하지 않습니다.

### 10.2 A: 기존 직접 접속 유지

기존 방식을 유지하기로 했다면 다음을 기록합니다.

| 항목 | 기록 내용 |
|---|---|
| 접속 주소 | 공인 IP 또는 DDNS 호스트명 |
| 공유기 규칙 | 외부 TCP 포트 → 내부 IP:VNC 포트 |
| 내부 IP 유지 | DHCP 예약 또는 고정 주소 |
| 허용 출발지 | 적용 중인 원격 주소 제한 |
| 암호화 | VNC TLS / 외부 터널 / 없음 / 미확인 |
| IPv6 | 별도의 수신·방화벽 정책 |

외부 VNC 포트의 신규 전면 개방은 기본 설치 명령에 넣지 않습니다. 기존 경로 기록과 보안 검증은 구분합니다. 기준 PC 실행 명령에는 VNC 자체 TLS 옵션이 없습니다.

공유기 권한 부족이나 이중 NAT·통신사 NAT가 있으면 포트포워딩만으로 해결된다고 가정하지 않고 SSH·VPN을 검토합니다.
...
- **VPN 안의 SSH 터널:** 10.3의 SSH_HOST에 VPN 주소를 사용. VNC는 `-localhost` 유지.
...
- 저장되지 않은 작업과 적용 시점을 정리했고 SSH 복구 경로가 있다.
...
| 외부망 | 테더링 등 별도 네트워크 | 선택한 외부 경로로 인증·화면·입력 성공 |
...
| Tailscale active인데 안 됨 | NeedsLogin·IP·접근 정책 | 실제 로그인·경로 확인 |
| VPN에서 VNC만 안 됨 | -localhost 여부 | SSH 터널 또는 VPN 수신 구성 확인 |
