...
[키보드 모드 매뉴얼](KEYBOARD-MODE.md)에 Caps Lock→한/영 전환, 기존 설정 복구, Mac/iPad 입력 언어와 조합 문제 점검 절차를 정리했습니다. 바탕화면 ‘VNC 키보드 모드’에서 켜고 끕니다. Mac 기본 ‘화면 공유’의 Cmd+C/V 복사·붙여넣기, 낱자 조합 보정과 iPad RVNC의 Caps 잠금 상태 보정 모듈도 포함합니다. iPad 모듈 최초 설치에는 관리자 인증과 VNC 재접속 한 번이 필요합니다.

iPad RVNC에서 복사한 한글이 깨지면 **iPad 복사 보호**를 켭니다. Ubuntu 내부 복사를 보호하며, 켜져 있는 동안 기기에서 Ubuntu로 보내는 클립보드는 제한됩니다. 같은 바탕화면 창에서 끌 수 있습니다.
