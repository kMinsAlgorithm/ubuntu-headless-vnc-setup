...
## iPad 화면 키보드에서 특수기호가 나올 때

물리 키보드와 화면 키보드의 전달 형식이 다를 수 있습니다. 실제 RVNC 화면 키보드 진단에서 한글 자모가 X11 Unicode 접두어 없는 `0x3141`, `0x3160`, `0x314a` 등으로 도착했습니다. 기존 Unicode keysym/legacy Hangul 보정에 이 형식이 빠져 있었습니다.

VNC 모드는 같은 단일 두벌식 자모 33개에 대해 이 코드 별칭도 영문 키로 연결합니다. `0x3141`(ㅁ)→a, `0x3160`(ㅠ)→b, `0x314a`(ㅊ)→c 후 Ubuntu IBus에서 한글을 조합합니다. 복합 자모·완성된 음절·다른 Unicode 전체를 임의 변환하지 않습니다. 직접 사용 시 원래 매핑을 복구합니다.

키 이벤트가 도착했다는 사실만으로 정상 문자 입력이라고 판단하지 않습니다. 지정한 소량의 키 코드, 입력 결과, 입력기 상태를 구분합니다. 이 사례는 클립보드 보호가 꺼진 상태에서도 재현됐으므로 복사 보호 문제와 구분합니다. 갱신에는 관리자 인증·Caps 모듈 재빌드·VNC 재시작이 필요 없습니다. 한글 대신 영문이 나오면 Shift+Space로 Ubuntu 입력 소스를 한 번 맞춥니다.

수정 후 실제 iPad 화면 키보드 입력 정상 확인을 받았습니다. 이후 복사 보호도 다시 켠 상태로 유지했습니다. 두 기능은 별개이며, 입력 문제를 진단하면서 보호를 껐다면 복사 시험 전에 다시 켜져 있는지 확인합니다.
